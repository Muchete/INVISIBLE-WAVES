...
{      
  ...
  //start bridgne for writing to sd
  Bridge.begin();
  ...
  //NEW DATASETPRINT
  File file = FileSystem.open("/mnt/sd/data.txt", FILE_APPEND);
  if(file){

    file.println();
    file.println();
    file.println("NEW DATASET");

    file.close();
  } else {
    Serial.println("no file! Whats going on dude?");
  }
...
void loop()
{
  ...
  pixels.setPixelColor(led_count_minus(led_count, 0), pixels.Color(brightness/16, brightness/16,brightness/16));
  pixels.setPixelColor(led_count_minus(led_count, 1), pixels.Color(brightness/4, brightness/4,brightness/4));
  pixels.setPixelColor(led_count_minus(led_count, 2), pixels.Color(brightness/1, brightness/1,brightness/1));
  pixels.setPixelColor(led_count_minus(led_count, 3), pixels.Color(brightness/4, brightness/4,brightness/4));
  pixels.setPixelColor(led_count_minus(led_count, 4), pixels.Color(brightness/16, brightness/16,brightness/16));
  pixels.setPixelColor(led_count_minus(led_count, 5), pixels.Color(0, 0, 0));
...
//led ring functions
int led_count_minus (int count, int steps)
{
  if (steps == 0)
  {
    return count;
  ...
  {
    return led_count_minus(led_count_minus(count, 1), steps - 1);
  }
}
...
int MMA7455_init(void)
{
  ...
  c1 = bit(MMA7455_GLVL1) | bit(MMA7455_GLVL0) | bit(MMA7455_MODE0);
